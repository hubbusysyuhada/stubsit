...
export default class MasterController {
  ...
  static async newGroup(
    ...
    const { data: group } = await request.supabase
      .from('groups')
      .select('id')
    ...
    await request.supabase.from('groups').insert([
      {
  ...
  static async deleteGroup(
    ...
    if (group) {
      if (group.endpoints.length)
        return reply.code(400).send(new Error('Group still has endpoints.'));
      await request.supabase.from('groups').delete().eq('id', group.id);
  ...
  static async newEndpoint(
    ...
    const { error, data } = await request.supabase
      ...
      .single();
    if (data) {
      const methods: Database['public']['Enums']['http_method_type'][] = [
        'GET',
        'POST',
        'PUT',
        'DELETE',
      ...
      for (const method of methods) {
        await request.supabase.from('calls').insert([
          {
            endpoint_id,
            method,
            response_code: 200,
  ...
  static async editEndpoint(
    request: FastifyRequest<{
      Body: { name: string; description: string; group: string };
      Params: { endpoint: string; group: string };
    }>,
    reply: FastifyReply
  ) {
    const { data: group } = await request.supabase
  ...
  static async deleteEndpoint(
    request: FastifyRequest<{ Params: { endpoint: string; group: string } }>,
    reply: FastifyReply
  ) {
    const { data: endpoint } = await request.supabase
      .from('endpoints')
      .select('id')
      .eq('slug', request.params.endpoint)
      .single();
    if (endpoint) {
      await request.supabase.from('calls').delete().eq('endpoint_id', endpoint.id);
      await request.supabase.from('endpoints').delete().eq('id', endpoint.id);
      return reply.code(200).send({ data: 'delete success' });
    }
    return reply.code(400).send(new Error('Something went wrong.'));
  }
  ...
  static async getEndpointBySlug(
    ...
    const { data } = await request.supabase
      .from('endpoints')
      .select('*, calls(slug,method,is_error,response_code)')
      .eq('slug', request.params.endpoint)
      .order('id', { foreignTable: 'calls' })
      .single();
  ...
  static async deleteCall(
    request: FastifyRequest<{ Params: { endpoint: string; call: string } }>,
    reply: FastifyReply
  ) {
    const supabase = request.supabase;
    const { data: endpoint } = await supabase
  ...
  static async getCallBySlug(
    request: FastifyRequest<{ Params: { endpoint: string; call: string; group: string } }>,
    reply: FastifyReply
  ) {
    const { data, error } = await request.supabase
      .from('calls')
      .select(
        'method,slug,response_code,is_error,error_message,response,endpoint:endpoints!inner(slug)'
      )
      .eq('slug', request.params.call)
      .eq('endpoints.slug', request.params.endpoint)
      .single();
